The LC-900 Assembler and Simulator
===============

To aid in testing your processor, we have provided an *assembler* and
a *simulator* for the LC-900 architecture. The assembler supports
...
the assembler. The LC-900 assembler definition is included.

Included Files
-----------

* `assembler.py`: the main assembler program
* `lc900.py`: the LC-900 assembler definition
* `lc900-sim.py`: the LC-900 simulator program
...
Example usage to assemble `test.s`:
    ...
    python assembler.py -i lc900 --sym test.s
...
To output assembled code in hexadecimal (for use with *CircuitSim*):
    ...
    python assembler.py -i lc900 --hex test.s
